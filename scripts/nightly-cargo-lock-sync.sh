...
# Retries for `gh pr create` — see ensure_pr for the failure these exist for.
# Three attempts over ~15s: enough for an API blip, short enough that a
# genuinely broken repo still reports within the job's normal runtime.
PR_CREATE_TRIES="${PR_CREATE_TRIES:-3}"
PR_CREATE_RETRY_SEC="${PR_CREATE_RETRY_SEC:-5}"
...
# ── Find this nightly's open bot PR, or open it. Echoes the PR URL.
#
# `--base develop` on the lookup matches what the create below opens, and keeps
# a same-named branch on a fork — or a bot-branch PR aimed at main — from being
# adopted here and then merged by reap_pending.
#
# The create is RETRIED, because a single transient API error used to fail the
# whole job, and this job is what reports the nightly. Observed 2026-08-10 in
# run 31373922024: for greentic-start, and for none of the other 30 repos in
# that same run, `gh pr create` answered
#
...
# (b64a7eb0, 12:00:02, greentic-ci[bot]) and one night after the same repo
# opened #499 without trouble. Access was not the problem: the PR opened by hand
# minutes later as #501.
#
# Each attempt re-checks for an open PR FIRST. That is not redundant with the
# initial lookup — a create can succeed and still look like a failure (a dropped
# response, output `tail -1` does not catch), and a retry that went straight
# back to `gh pr create` would open a duplicate.
ensure_pr() {
  local full="$1"
  local body="$2"
  local attempt existing create_out=""

  for (( attempt = 1; attempt <= PR_CREATE_TRIES; attempt++ )); do
    existing=$(gh pr list --repo "$full" --head "$BRANCH" --base develop --state open \
                --json number --jq '.[0].number // empty' 2>/dev/null || true)
    if [[ -n "$existing" ]]; then
      echo "https://github.com/${full}/pull/${existing}"
      return 0
    fi
    ...
    if (( attempt < PR_CREATE_TRIES )); then
      warn "${full##*/} PR create attempt $attempt failed, retrying: $create_out"
      sleep "$PR_CREATE_RETRY_SEC"
    fi
  done

  err "[${full##*/}] PR create failed after $PR_CREATE_TRIES attempts: $create_out"
  return 1
}
...
EOF
  ...
  if ! out_pr_url=$(ensure_pr "$full" "$body"); then
    out_status="failed"
    return 0
  fi
