#!/usr/bin/env bash
# sync-rnd-publish.sh — Deploy rnd-publish caller workflows to repos
#
# For each repo with non-empty `publishes` in REPO_MANIFEST.toml, generates
# a thin rnd-publish.yml caller workflow on the `develop` branch that
# delegates to the shared reusable workflow.
#
# Usage (from the workspace root, one level above the .github checkout):
#   bash .github/scripts/sync-rnd-publish.sh               # create PR targeting develop
#   bash .github/scripts/sync-rnd-publish.sh --dry-run     # show what would change, change nothing
#   bash .github/scripts/sync-rnd-publish.sh --direct      # commit directly to develop, push
#   bash .github/scripts/sync-rnd-publish.sh --check       # exit non-zero if any repo drifts
#   bash .github/scripts/sync-rnd-publish.sh --repo NAME   # single repo only
#   bash .github/scripts/sync-rnd-publish.sh --tier N      # single tier only
#
...
SINGLE_REPO=""
SINGLE_TIER=""
TARGET_BRANCH="research"
SYNC_BRANCH="chore/sync-rnd-publish"
COMMIT_MSG="chore: sync rnd-publish caller from REPO_MANIFEST.toml"
...
for name, entry in m.get('repos', {}).items():
    if entry.get('archived', False):
        continue
    # The rnd (research) lane is gated per-repo via its own flag so it rolls out
    # independently of the dev lane. A repo stays dormant until opted in.
    if not entry.get('rnd-publish-enabled', False):
        continue
    # crates: rnd-specific override, else reuse the dev publishes list. A
    # research library whose crates.io graph is dirty (git deps / publish=false)
    # must NOT appear here -- set rnd-crates to the publishable subset.
    publishes = entry.get('rnd-crates', entry.get('publishes', []))
    if not publishes:
        continue
    ...
    binary = ' '.join(entry.get('rnd-binary-crates', entry.get('binary-crates', []))) or '_NONE_'
    # Research binary crates are binary-ONLY by default: their real library cannot
    # reach crates.io (git deps / publish=false aw-runtime), which is the very
    # reason the <name>-rnd shim exists. dual-role is strictly OPT-IN via the
    # rnd-dual-role-binary-crates field -- reserved for a binary whose library
    # has a genuinely clean crates.io graph. See RND_LANE.md.
    dual   = ' '.join(entry.get('rnd-dual-role-binary-crates', [])) or '_NONE_'
...
generate_caller() {
  ...
  cat <<EOF
# Auto-generated by sync-rnd-publish.sh — do not edit manually.
...
jobs:
  # Stage 1 — checks + version. Unlike the dev lane there is NO RUN_ID
  # stamping: the version is taken verbatim from Cargo.toml (X.Y.Z-research)
  # and validated for the -research suffix, then consumed by the binaries and
  # publish jobs below.
  rnd-prepare:
    uses: greenticai/.github/.github/workflows/rnd-prepare.yml@main
EOF

  # Only emit `with:` when at least one input needs setting (empty `with:` is
  # valid YAML but noisy to actionlint). rnd-prepare has no dual-role /
  # version-stamping knobs — verbatim -research versioning removes the dev
  # lane's require-pre-release entirely.
  ...
  # One job per binary crate so each target's failure is independent. The
  # archive is named for the verbatim -research version and matches the shim's
  # [package.metadata.binstall], so `cargo binstall <crate>-rnd --version
  # <X.Y.Z-research>` resolves it (the --version pin is required — research
  # versions are pre-releases; see greentic-dev#206).
  local binary_job_ids=""
  if [[ -n "$binary_crates" ]]; then
    for pkg in $binary_crates; do
      local job_id="rnd-binaries-${pkg}"
...
EOF
      ...
      # name (per `binary-bins` in REPO_MANIFEST.toml). rnd-release-binaries.yml
  ...
  # Binary crates publish as the dependency-free `<name>-rnd` shim (the real
  # research binary can't publish — git deps + publish=false aw-runtime);
  # dual-role stays empty unless a binary's library genuinely publishes.
  local publish_needs="[rnd-prepare${binary_job_ids:+, ${binary_job_ids}}]"
  cat <<EOF
...
sync_repo() {
  ...
  local workflow_path=".github/workflows/rnd-publish.yml"
  ...
  if [[ "$MODE" == "pr" ]]; then
    ...
    pr_url=$(gh pr create \
      ...
      --head "$SYNC_BRANCH" \
      --title "$COMMIT_MSG" \
      --body "Syncs \`rnd-publish.yml\` caller workflow from \`REPO_MANIFEST.toml\`.
...
echo -e "${BOLD}Greentic Rnd-Publish Sync${RESET}"
